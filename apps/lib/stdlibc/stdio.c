...
static FILE *_createstream(int fh,int oflag) {
 ...
 */
void stdio_init() {
  stdin = _createstream(STDIN_FILENO,O_RDONLY);
  stdout = _createstream(STDOUT_FILENO,O_WRONLY);
  stderr = _createstream(STDERR_FILENO,O_WRONLY);
  tmp_num = 0;
  atexit(stdio_exit);
}
 ...
 *  @todo fix
 */
static int mode_str2num(char *mode) {
  int m;
  if (mode[0]=='r') m = O_RDONLY;
  else if (mode[0]=='w') m = O_WRONLY;
...
FILE *fdopen(int fh,const char *mode) {
  return _createstream(fh,mode_str2num((char*)mode));
}
...
FILE *fopen(const char *name,const char *mode) {
  int m = mode_str2num((char*)mode);
  int fh = open(name,m);
  if (fh>=0) {
    if ((m&O_TRUNC)==O_TRUNC) ftruncate(fh,0);
    return _createstream(fh,m);
  }
  return NULL;
}
...
int fflush(FILE *stream) {
  if (stream->dobuf) {
    ...
    else ret = write(stream->fh,stream->buf,stream->bufcur-stream->buf)==-1?EOF:0;
    stream->bufcur = stream->buf;
    return ret;
  }
  return 0;
